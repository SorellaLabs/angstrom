...
The contract exposes two external functions: a setter callable by either `owner()` or `fastOwner()` of Angstrom's current `ControllerV1` contract, and a view getter returning both rates in a single call. The setter updates both LP shares atomically and emits their old and new values. Each protocol share is the complement of its corresponding LP share. Node reads must obtain both rates from the same pinned parent state, whether through the getter or by decoding storage directly.
...
contract OffchainProtocolFeeConfig {
    ...
    function getLpDonationSplits()
        external
        view
        returns (uint32 userLpShareE6_, uint32 tobLpShareE6_)
    {
...
`IControllerOwners` describes the existing controller getters; it does not add external functions to `OffchainProtocolFeeConfig` beyond the setter and the pair getter.

Keep the state variables and the immutable Angstrom reference private and expose the pair through `getLpDonationSplits()`. The reason to keep the fields private is not to minimize the ABI but to keep a single read path that returns both rates together: two automatically generated single-value getters would let a caller read the two rates in two separate calls, which can straddle a block boundary and produce a mixed pair. One getter returning both, and one storage word holding both, each satisfy the same-parent-state requirement. There are no inherited external functions, fallback, receive, ownership-transfer, or withdrawal functions on `OffchainProtocolFeeConfig`.

The immutable Angstrom reference consumes no storage slot. The two `uint32` shares are packed into slot zero, with the first field in its least-significant four bytes. Direct storage decoding remains a supported read path alongside the getter, so treat this layout as part of the contract's interface and verify it against compiler storage-layout output. See the [Solidity storage-layout documentation](https://docs.soliditylang.org/en/v0.8.26/internals/layout_in_storage.html) and [contract documentation](https://docs.soliditylang.org/en/v0.8.26/contracts.html).
...
| Bundle telemetry, historical fee reporting, and the fee-accounting component identified during implementation | Record gross ToB payment, LP allocation, explicit protocol fee, rounding residual, and snapshot identity. Reconstruct canonical included allocations and reconcile protocol proceeds to `Asset.save` and the controller's existing distribution calls. Integrate an identified external ledger or implement this component; its existence is not assumed. Accounting and reconciliation only; payout scheduling is out of scope. |

Resolve all callers of the changed bundle/matching interfaces, including mocks, benchmark helpers, testnet setup, and replay. Update the relevant bundle-building documentation and add an operational procedure for reading the current pair and submitting the setter. Updating `PLAN.md` itself does not implement or deploy any of these changes.

## Tracking canonical state

Read both shares at each canonical head using the local provider and an explicit block hash. Either read path is acceptable: decode storage slot zero, which holds both shares in one word, or call `getLpDonationSplits()`, which returns both in one call. Whichever path is used, a single read must yield both rates from one pinned parent state; do not compose a pair from two reads. Keep read logic and validation in the configuration-tracking module. An RPC-backed implementation can use `eth_getStorageAt` or `eth_call`, each with an EIP-1898 block-hash identifier and `requireCanonical: true`, for live construction. Require support for hash-pinned state reads; do not substitute an unpinned `latest` or block-number-only read. See the [Ethereum JSON-RPC documentation](https://ethereum.org/developers/docs/apis/json-rpc/#eth_getstorageat) and [EIP-1898 block-hash identifiers](https://eips.ethereum.org/EIPS/eip-1898).
...
The repository provides the owner-only withdrawal mechanism, but no source-specific fee ledger has been identified here. Treat this as an explicit implementation dependency: identify and integrate an external component, recording its repository/service, release, responsible operator, and operating procedure in the rollout artifacts, or implement the required ledger and reporting tooling as part of this work.

### Payout scope

Accurate fee accounting and reconciliation are required by this release. **Automated treasury payout scheduling is not part of it.** This release performs no scheduled, triggered, or otherwise unattended withdrawal. Every distribution is an operator-reviewed action executed through the existing governance path: the controller's `owner()` — the timelock, in the mainnet deployment configured by [`contracts/script/Angstrom.s.sol`](contracts/script/Angstrom.s.sol) — calling the owner-only `ControllerV1.distributeFees` through its existing scheduling and execution process.

The division of labor is therefore:

| Concern | This release | Later work |
| --- | --- | --- |
| Accrual, reconciliation, and reporting | Required | — |
| Proposing a reconciled, uncollected amount with its evidence | Required | — |
| Reviewing and approving that proposal | Human operator | — |
| Executing the withdrawal | Timelock owner via `distributeFees` | — |
| Scheduling or automatically triggering a withdrawal | Out of scope | Separate work |

No component built in this rollout holds withdrawal authority or initiates a distribution on its own; the accounting component's output is a reviewable proposal, not an instruction. The fast owner cannot distribute at all. Do not implement any part of the ledger in a way that presumes a later scheduler, and do not grant a service the owner key as a shortcut to one.

Before enabling a nonzero ToB protocol share, identify both the accounting component and its responsible operator by name, and record them in the rollout artifacts: repository or service, release, the operator accountable for reviewing and executing distributions, and the operating procedure. An unidentified component or an unassigned operator blocks the nonzero ToB fee. Neither blocks activation at the initial economics, where the ToB protocol share is zero.

That component must demonstrate all four of the following before the fee is enabled:

1. **Reconcile against actual included bundles.** Derive every accrual from canonical included bundles and their construction parent state, not from proposal or submission telemetry.
2. **Preserve LP and user backing.** Show that a proposed withdrawal leaves LP rewards and internal user balances fully backed. Angstrom's ERC20 balance is not the withdrawable amount.
3. **Handle reorgs.** Undo and re-derive accruals from removed blocks, and correct ledger records already advanced past them.
4. **Prevent duplicate withdrawals.** Persist accrual and distribution identities so restart, backfill, and reorg processing cannot collect the same fee twice, and so a re-reviewed proposal cannot double-spend an amount already distributed.

Establish a reconciled ledger checkpoint through A-1 before activation, keyed by canonical block number/hash and token. Record accrued, already distributed, and uncollected amounts from existing fee sources; the new ToB percentage-fee balance starts at zero. Historical unresolved amounts must remain separately identified and excluded from proposed distributions. Persist accrual and distribution identities so restart, backfill, or reorg processing cannot count them twice; advance the checkpoint with canonical included data rather than proposal or submission telemetry alone.

Use the owner's existing `ControllerV1.distributeFees` call, under operator review, to withdraw only reconciled, uncollected fees and direct the ToB component to the protocol treasury. `pullFee` trusts the controller's requested amount rather than maintaining a new accrued-ToB balance. Do not infer withdrawable fees from Angstrom's entire ERC20 balance, which also backs LP rewards and internal user balances. Record completed distributions and undo/reconcile records affected by reorgs to avoid collecting the same fee twice.

The configuration contract holds only the two rates and no fee funds. Treasury selection and payout execution belong to the identified accounting/operational workflow using the existing controller withdrawal function; adding a treasury setter or payout function to the configuration contract would extend it beyond configuration storage into fund custody and is unnecessary here.
...
- Contract interface: compiled ABI has exactly two externally callable functions, `setLpDonationSplits(uint32,uint32)` and `getLpDonationSplits()`, with no fallback/receive entry points and no ownership-transfer or withdrawal functions. The getter is `view`, returns both rates from one call, and is callable by anyone. No existing contract ABI or runtime bytecode needs to change.
...
- Storage and getter agreement: verify both field offsets in slot zero and Rust decoding against the compiled contract, and assert that `getLpDonationSplits()` and slot-zero decoding return the same pair at the same block hash, including immediately after an update. Cover zero values, invalid values, nonzero padding, missing code, mismatched immutables, and a one-value config contract at the configured address. Whichever read path the node uses, assert both rates come from one pinned parent state rather than two reads.
...
- Treasury: exercise the identified or newly implemented accounting component, its A-1 opening checkpoint, canonical included-bundle ingestion, restart/backfill, and reorg corrections. Exercise the four requirements in Payout scope directly: reconciliation against canonical included bundles, LP/user backing preserved by a proposed withdrawal, reorg undo and re-derivation, and refusal to distribute an amount already distributed across a restart, a backfill, and a re-reviewed proposal. Distribute the new proceeds to a treasury through the existing owner-only controller function, as an operator-reviewed timelock execution rather than an unattended call; show that LP and user liabilities remain backed. Verify a fast owner that is not also owner cannot distribute fees, and that no component in this rollout can initiate a distribution on its own. Reconcile fee-summary data and exclude unresolved historical amounts and policy mismatches from proposed distributions.
...
The requirements above are stated as design. Design alignment does not establish implementation correctness, and agreement that this plan is right is not evidence that the code matching it is right. Each of the following must arrive as concrete evidence in the pull requests that implement it — a named, runnable test that fails when the requirement is violated — not as a claim in a description or a review comment. A PR that changes one of these paths without its evidence is incomplete.

### Required evidence

1. **Identical parent state and rates through simulation and final construction.** A test that drives one round end to end and asserts that gas estimation and final bundle construction consumed the same `DonationSplitSnapshot` and the same parent hash. It must fail if either path re-reads configuration or pool state independently. Change the canonical head partway through the round and assert the stale result is rejected rather than silently used, including a same-height reorg where the height alone still matches.

2. **Cancellation of invalidated local submission work.** A test that invalidates a round while matching, simulation, nonce/gas RPCs, signing, or an endpoint send is in flight, and asserts that no subsequent local send or retry occurs. Assert against the observable effect — sends that did not happen — rather than the presence of a cancellation token. Cover the current failure shape explicitly: a spawned task whose join handle was dropped must not proceed to sign or submit. State the accepted limitation in the same test module: a transaction already sent is not recalled, and that case is covered by the reconciliation evidence in item 3 instead.

3. **Conservation, with residuals explained and nothing counted twice.** Per fee source, assert `sum(encoded LP donations) + explicit protocol fee + allocation residual == gross`, with every value nonnegative and the residual attributed to the documented integer-allocation steps rather than accepted as a leftover. Assert no double counting across the boundary the plan identifies: the saved amount is both allocated and reserved, so `collect_extra` cannot count it again, and gas accounting at its existing call sites remains separate from retained fees. Reject unexplained remainders regardless of size; no "material" threshold.

4. **Generated-bundle execution against the unchanged contracts.** Execute bundles produced by the real builder against the existing Angstrom contracts in the Anvil/Foundry harness. Assert exact saved amounts with gas and residuals each accounted for once, zero unresolved bundle deltas, and expected LP reward growth subject to existing fixed-point rounding. Fixtures hand-written to the expected encoding do not satisfy this item; the bundle must come from the code path being shipped.

### Coverage that must not be omitted

These are the paths most likely to be skipped, and each has already caused a requirement in this plan:

- **Empty and no-op paths.** True no-ops for both sources, with and without positive active liquidity; book-only exact-match batches carrying positive user fees; book no-ops following a ToB move, allocated at post-ToB state rather than the parent price; zero budgets that still preserve required swap and tick/liquidity metadata; and `Some(empty)` vectors, which must not bypass current-range allocation and silently retain LP rewards.
- **Multiple pools sharing a token.** At least one test with two pools sharing token0, asserting that shares are applied once per pool's gross total rather than per tick, fragment, or post-aggregation asset, and that checked accumulation across pools rejects rather than wraps or saturates.

### Staged activation

Activation is two steps, not one, and the second is gated on evidence from the first.

1. Activate at block **A** with the existing economics — `userLpShareE6 = 750_000`, `tobLpShareE6 = 1_000_000` — so the only live changes are the configuration source, the integer arithmetic, and the new allocation and error paths. Verify construction, settlement, and accounting against real canonical blocks: snapshots resolve to the intended parent for every round, post-settlement reconstruction matches included reward and saved amounts, and the ledger reconciles against actual included bundles.
2. Only then call the setter to enable the chosen ToB share, with the accounting component and its operator identified per Payout scope.

Do not combine the two. Activating the new code path and a new economic parameter in one step leaves any resulting discrepancy ambiguous between them.

## Implementation sequence

1. Define the validated pair of shares and its arithmetic, then implement `OffchainProtocolFeeConfig`, its setter and pair getter, contract tests, ABI bindings, and a standalone deployment script. Verify both read paths and their agreement before implementing node decoding.
...
5. Prepare the deployment and activation artifacts: constructor arguments, verified runtime/layout, network address, deployment block/hash, activation target block, node release, named fee-accounting component and operator, opening-ledger procedure, and operator-reviewed governance execution and treasury distribution procedures. Land the Implementation acceptance evidence with the PRs that implement each path, and plan activation as the two gated steps in Staged activation. Record the limits of peer verification and already-submitted transaction cancellation.
...
2. Deploy only `OffchainProtocolFeeConfig` with constructor arguments `(existingAngstrom, 750_000, 1_000_000)`. Verify both resolved controller authorities and their existing execution paths, runtime code, storage layout, and initial values, and confirm the getter and slot-zero decoding agree at the deployment block hash. No controller handover or existing-contract redeployment is needed.
...
6. Once participating nodes use the new flow and step 4 has produced its verification evidence, have either the existing controller's owner or fast owner call `setLpDonationSplits(currentUserShare, chosenTobShare)`. A multisig fast owner can execute the call directly; a timelock owner uses its existing scheduling and execution process with `OffchainProtocolFeeConfig` as the target. Read and review both supplied values to avoid resetting an unrelated share. The final values at the setter's canonical block H govern construction for H+1.
7. Reconcile retained ToB fees in the identified fee-accounting component, have the named operator review the resulting proposal, and have the controller owner execute the distribution to the chosen treasury through the timelock. Exclude unreconciled policy mismatches or missing historical data from that proposal. Changing the rate does not perform a payout, and no payout in this release is scheduled or automatic.
