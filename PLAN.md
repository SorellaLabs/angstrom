...
| [`crates/consensus/src/rounds/proposal.rs`](crates/consensus/src/rounds/proposal.rs) | Pass the same round snapshot into final proposal bundle construction. |
...
| Bundle telemetry, historical fee reporting, and existing payout tooling | Record gross ToB payment, LP allocation, explicit protocol fee, rounding residual, and snapshot identity. Reconcile protocol proceeds to `Asset.save` and the controller's existing distribution calls. |
...
6. Do not build a round with stale configuration when a required read fails. Retry or skip the affected round according to the node's error-handling policy.
...
Capture the snapshot once for the round, after the relevant block state is ready, and keep it fixed throughout matching, gas estimation, and final bundle construction. A new head or reorg must invalidate the previous round's work through the round lifecycle.
...
Check that configuration and pool snapshots refer to the intended same parent state before starting and before submitting the result. Discard results from invalidated rounds, including same-height reorgs. Where proposal validation or independent reconstruction checks the split policy, derive it from the same parent snapshot rather than trusting a rate supplied by the proposing node. A successful EVM simulation alone does not validate this policy.
...
This also removes the existing floating-point precision loss. Even at 75%, large quantities may round differently from the old implementation. Treat the arithmetic change as part of the coordinated activation.
...
- Reconcile the sum of the ToB donation vector against `tob_lp_budget` before merging it with book donations. The current `t0_donation_vec` can leave a residual after its integer price calculations; the builder's `collect_extra` can retain residual liquidity. Preserve this existing residual policy and record the amount separately from the explicit ToB percentage fee. Do not assert that the nominal LP budget is always the exact encoded donation total.
- Verify `encoded_tob_lp_donation + tob_protocol_fee + tob_allocation_residual == gross_tob_reward`, with all values nonnegative. Reject over-allocation. Treat a material unallocated amount caused by an empty or invalid donation path as an allocation failure, not as an ordinary rounding residual. The implementation must identify the residual from allocator output rather than label arbitrary final contract balances as dust.
- Handle no-op or empty donation vectors explicitly. A positive LP budget must have a valid reward recipient range with positive liquidity; use a valid current-range allocation for a no-op when appropriate, or reject the affected construction. Do not silently save the entire LP budget because the allocator returned no ranges.
- At `tobLpShareE6 == 0`, preserve required swap and tick/liquidity metadata even though ToB reward amounts are zero. The ToB order and any book rewards still settle normally. At `1_000_000`, the explicit ToB protocol fee is zero and the established ToB donation behavior remains the baseline.
...
`Asset.save` aggregates gas, retained user fees, the new ToB protocol fee, and any existing residual retention. It has no source-specific sub-balances or automatic treasury routing. Extend the existing off-chain fee report/payout accounting to track the explicit ToB fee per pool, token, bundle, and parent snapshot, aggregate by token for collection, and reconcile the result against saved amounts and the existing fee-summary log commitment. Account for rounding residuals separately and follow the existing distribution policy for other fee sources.
...
The configuration contract holds only the two rates and no fee funds. Treasury selection and payout execution remain in the existing distribution workflow; adding a treasury setter or payout function would violate the one-external-function requirement and is unnecessary here.
...
- Treasury: distribute the new proceeds to a treasury through the existing owner-only controller function; show that LP and user liabilities remain backed. Verify a fast owner that is not also owner cannot distribute fees. Reconcile fee-summary data, duplicate-collection prevention, and reorg corrections.
- Replay: reconstruct both shares from the correct historical parent state and apply an explicit policy for blocks predating deployment or activation, including the old floating-point user-fee calculation when reproducing historical bundle bytes.
...
2. Implement hash-pinned canonical-state loading and block-synchronized snapshot publication. Wire startup, commits, reorgs, gaps, and failures before allowing rounds to consume configuration.
3. Thread one snapshot through consensus, matching, gas finalization, and final bundle construction. Replace the user-fee constant and apply the ToB split and asset accounting at the shared `process_solution` entry point.
4. Add donation/residual reconciliation and fee reporting, update test/replay/address initialization, and validate full bundles against the unchanged settlement contracts. Keep this as one coordinated behavior change across construction paths rather than enabling one path early.
5. Prepare the deployment and activation artifacts: constructor arguments, verified runtime/layout, network address, deployment block/hash, activation target block, node release, and the existing governance execution and treasury distribution procedures.

## Rollout

1. Implement and validate the configuration contract, owner and fast-owner lookups, bindings, canonical tracking, both split calculations, retained-fee accounting, and node integration.
...
4. Coordinate the transition to chain-derived configuration and integer arithmetic for bundles targeting A and later. Keep initial LP shares at 75% for user fees and 100% for ToB during activation. Observe agreement between simulation, construction, saved-fee reports, and settlement before enabling a ToB fee.
5. For replay of targets before A, retain the legacy user-fee floating-point behavior and full ToB LP budget when byte-exact reproduction is required. For targets at or after A, load both shares from their actual historical parent state. Deployment alone does not activate the new node policy. If a replay lacks required historical state, report the gap rather than silently applying today's setting.
...
7. Reconcile retained ToB fees in the existing payout workflow and have the controller owner distribute them to the chosen treasury. Changing the rate does not perform a payout.
...
No bundle encoding change is required to source either split from chain state or to retain the ToB protocol portion: the resulting donation and saved amounts already enter the encoded bundle. The new contract configures off-chain policy. Additional on-chain enforcement of the ratios, automatic treasury forwarding, or changes to existing Angstrom/controller/governance contracts are outside this plan.
