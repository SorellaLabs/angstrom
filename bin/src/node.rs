...
pub struct NodeCommand<Ext: RethCliExt = ()> {
    ...
    ///
    /// Max number of instances is 200. It is chosen in a way so that it's not
    /// possible to have port numbers that conflict with each other.
    ///
...
impl<Ext: RethCliExt> NodeCommand<Ext> {
    ...
    ///
    /// By default this will be a [BeaconConsensus] instance, but if the `--dev`
    /// flag is set, it will be an [AutoSealConsensus] instance.
    ...
    /// Spawns the configured network and associated tasks and returns the
    /// [NetworkHandle] connected to that network.
    ...
    /// Attempt to look up the block with the given number and return the
    /// header.
    ///
    /// NOTE: The download is attempted with infinite retries.
    ...
    fn load_network_config(
        ...
        // When `sequencer_endpoint` is configured, the node will forward all
        // transactions to a Sequencer node for execution and inclusion on L1,
        // and disable its own txpool gossip to prevent other parties in the
        // network from learning about them.
        #[cfg(feature = "optimism")]
...
/// Drives the [NetworkManager] future until a
/// [Shutdown](reth_tasks::shutdown::Shutdown) signal is received. If
/// configured, this writes known peers to `persistent_peers_file` afterwards.
