...
use alloy_primitives::{Address, U256, address, aliases::U24, b256};
use angstrom_types::{
    contract_payloads::angstrom::AngstromPoolConfigStore, matching::SqrtPriceX96, primitive::PoolId
};
...
pub(crate) async fn run_devnet(executor: TaskExecutor, cli: DevnetCli) -> eyre::Result<()> {
    ...
    tracing::error!("{pairs_to_pools:?}");

    let new_pool_key =
        PartialConfigPoolKey::new(50, 60, 34028236692, SqrtPriceX96::at_tick(10000).unwrap());
    ...
    testnet.advance_block();
    testnet.check_pool_fees(
        AngstromPoolConfigStore::derive_store_key(
            address!("0xb45d2d77fedccf6cdc518ab94d212048c07ed963"),
            address!("0xdccee6dcacacc510fae48ecc37c2b3b8317a8a33")
        ),
        U24::from(1u8),
        U24::from(2u8),
        U24::from(4u8)
