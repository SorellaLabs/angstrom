...
pub mod test {
    ...
    // ===== TEMPORARY PROOF TEST (issue 3) =====
    #[tokio::test]
    async fn tmp_proof_late_subscriber_misses_startup_config_update() {
        let (mut eth, config_addr, mut early_rx, storage) = setup_config_eth_manager();
        let (canon_tx, canon_rx) = tokio::sync::broadcast::channel(8);
        eth.canonical_updates = BroadcastStream::new(canon_rx);
        let (command_tx, command_rx) = tokio::sync::mpsc::channel(8);
        eth.commander = ReceiverStream::new(command_rx);

        let setter_block = BlockHash::random();
        storage.holds(setter_block, (800_000, 900_000));

        // --- Case A: the subscribe command is ALREADY queued before the first poll.
        let (late_tx, mut late_rx) = tokio::sync::mpsc::unbounded_channel();
        command_tx
            .send(EthCommand::SubscribeEthNetworkEvents(late_tx))
            .await
            .unwrap();

        // Startup backlog carrying the setter, queued on the canon subscription.
        canon_tx
            .send(CanonStateNotification::Commit {
                new: chain(
                    101,
        ...
        // The cleanser itself moved to the new pair.
        println!("cleanser pair after poll = {:?}", eth.protocol_fee_config);
        assert_eq!(
            eth.protocol_fee_config.splits,
            DonationSplits::new(800_000, 900_000).unwrap()
        );
        // The listener registered before the poll got it.
        let early = published_config(&mut early_rx);
        println!("EARLY listener config event = {early:?}");
        assert!(early.is_some(), "listener present before the poll receives it");

        // The listener whose subscribe command was queued in the SAME poll did not:
        // the poll drains canonical_updates to Pending before it touches commands.
        let late_events_a: Vec<_> = std::iter::from_fn(|| late_rx.try_recv().ok()).collect();
        println!("CASE A late listener events = {late_events_a:?}");
        assert!(
            !late_events_a
                .iter()
                .any(|e| matches!(e, EthEvent::ProtocolFeeConfigUpdated(_))),
            "same-poll subscriber missed the config update"
        );
        // ...and it IS registered (proof the command was processed at all).
        println!("listener count after poll = {}", eth.event_listeners.len());
        assert_eq!(eth.event_listeners.len(), 2);

        // --- Case B: no self-heal. Later notifications without a setter publish
        // nothing, and nothing replays the current pair to the late listener.
        let plain = BlockHash::random();
        storage.holds(plain, (800_000, 900_000));
        canon_tx
            .send(CanonStateNotification::Commit { new: chain(102, plain, vec![]) })
            .unwrap();
        futures::future::poll_fn(|cx| {
            let _ = eth.poll_unpin(cx);
            Poll::Ready(())
        ...
        .await;
        let late_events_b: Vec<_> = std::iter::from_fn(|| late_rx.try_recv().ok()).collect();
        println!(
            "CASE B late listener events after a further block = {:?}",
            late_events_b
                .iter()
                .map(|e| match e {
                    EthEvent::ProtocolFeeConfigUpdated(s) => format!("Config({:?})", s.splits),
                    EthEvent::NewBlock(b) => format!("NewBlock({})", b.number),
                    other => format!("{other:?}")
                })
                .collect::<Vec<_>>()
        );
        assert!(
            !late_events_b
                .iter()
                .any(|e| matches!(e, EthEvent::ProtocolFeeConfigUpdated(_))),
            "late subscriber never self-heals"
        );
        println!(
            "FINAL: cleanser pair = {:?}, late subscriber ever told = {}",
            eth.protocol_fee_config.splits,
            late_events_a
                .iter()
                .chain(late_events_b.iter())
                .any(|e| matches!(e, EthEvent::ProtocolFeeConfigUpdated(_)))
        );
    }
}
