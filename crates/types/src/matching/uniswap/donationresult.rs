...
impl DonationResult {
    pub fn new(
        current_tick: u128,
        tick_donations: HashMap<Tick, u128>,
    ...
    pub fn combine(&self, rhs: &Self) -> eyre::Result<Self> {
        ...
        // direction matters here as we need to align with direction = false ticks.
        ...
        // direction doesn't matter once we combined
        Ok(Self { current_tick, tick_donations, final_price, total_donated })
    }

    pub fn low_tick(&self) -> Option<Tick> {
        self.tick_donations.iter().map(|(k, _)| k).min().copied()
    }

    pub fn high_tick(&self) -> Option<Tick> {
        self.tick_donations.iter().map(|(h, _)| h).max().copied()
    }
    ...
    pub fn far_tick(&self, start_tick: Tick) -> Option<Tick> {
        ...
        all_ranges.sort_by_key(|l| *l);
        all_ranges.last().map(|t| **t)
        // if all_ranges.len() == 1 {
        //     // If we only have one range, then short circuit
        //     return Some(*all_ranges[0]);
        // } else if all_ranges[0].0 <= start_tick && start_tick <
        // all_ranges[0].1 {     // If the first range is where we're
        // at, the last range is our 'far' range     return
        // all_ranges.last().map(|r| r.0); } else if let Some(last) =
        // all_ranges.last() {     if last.0 <= start_tick && start_tick
        // < last.1 {         // Otherwise, if the last range is where
        // we're at, the first range is our 'far'         // range
    ...
    pub fn donate_and_remainder(&self, net_swap: &PoolPriceVec) -> (Self, Option<Self>) {
        ...
        let (within_vec, outside): (HashMap<_, _>, HashMap<_, _>) = self
            .tick_donations
            .iter()
            ...
            .partition(|(t, _)| low < t && high >= t);

        let second_swap = if !outside.is_empty() {
            // let (cur_low, cur_high) =
            //     (net_swap.end_bound.liq_range.lower_tick,
            // net_swap.end_bound.liq_range.upper_tick);
