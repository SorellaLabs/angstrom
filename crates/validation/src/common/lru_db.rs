...
//     accounts:           Arc<RwLock<LruMap<Address, DbAccount,
// ByMemoryUsage>>>,     contracts:          Arc<RwLock<LruMap<B256, Bytecode,
// ByMemoryUsage>>>,     db:                 Arc<DB>,
...
// {
//     fn update_evm_state(&self, slot_changes: &AddressSlots) ->
// eyre::Result<()> {         let mut accounts = self.accounts.write();
//
...
// {
//     pub fn new(max_bytes: usize, db: Arc<DB>, current_block: Arc<AtomicU64>)
// -> Self {         let accounts =
// Arc::new(RwLock::new(LruMap::new(ByMemoryUsage::new(max_bytes))));
//         let contracts =
// Arc::new(RwLock::new(LruMap::new(ByMemoryUsage::new(max_bytes))));
//         Self {
//             current_block,
...
//     }
//
//     pub fn set_state_overrides(&self, overrides: HashMap<Address,
// HashMap<U256, U256>>) {         *self.state_overrides.write() = overrides;
//     }
//
//     pub fn set_bytecode_overrides(&self, overrides: HashMap<Address,
// Bytecode>) {         *self.bytecode_overrides.write() = overrides;
//     }
//
//     fn basic_ref_no_cache(&self, address: &Address) ->
// RethResult<Option<AccountInfo>> {         Ok(self
...
//     }
//
//     fn storage_ref_no_cache(&self, address: &Address, index: U256) ->
// RethResult<U256> {         self.get_current_provider()?
...
// Self::Error> {         DatabaseRef::basic_ref(&self, address)
//     }
//
//     fn storage(&mut self, address: Address, index: U256) -> Result<U256,
// Self::Error> {         DatabaseRef::storage_ref(&self, address, index)
//     }
//
...
// Self::Error> {         DatabaseRef::code_by_hash_ref(&self, code_hash)
//     }
// }
//
...
// Self::Error> {         let mut accounts = self.accounts.write();
//
//         accounts
//             .get(&address)
//             .map(|acc| Ok(acc.info()))
...
// Self::Error> {         unreachable!() // this should never be reached since
// the code hash is                        // defined in basic()
//     }
//
//     fn storage_ref(&self, address: Address, index: U256) -> Result<U256,
// Self::Error> {         // check for overrides
...
//         unreachable!() // this should never be reached since we will never
// sim                        // blocks
//     }
// }
