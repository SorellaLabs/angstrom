...
use crate::{
    ...
    types::{OpRpcBlock, WithWalletProvider, initial_state::DeployedAddresses}
};
    ...
    P: WithWalletProvider
{
    ...
    pub async fn execute_and_return_state(&self) -> eyre::Result<(Bytes, OpRpcBlock)> {
    ...
    pub async fn mine_block(&self) -> eyre::Result<OpRpcBlock> {
        let mined = self
            .provider
            .provider()
            .rpc_provider()
        ...
        self.provider.update_canon_chain(&block, recipts)?;
