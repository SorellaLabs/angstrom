# 32 — Empty-vector and unallocated-budget retention
...
1. **`Some(empty)` must not skip allocation silently.** Ticket 30 already routes the
...
2. **The `ucp.is_zero()` branch at `:392`** sets `book_swap_vec = None`, so
   ...
   record it: emit the amount as an `unplaced` residual for the book source so ticket 31's per-pool
   check still balances and ticket 37 can report it.
...
This ticket adds almost no code of its own — ticket 30's residual is the mechanism, and this is the
...
Ticket 36 asserts it rather than anything needing to change here.
